...
import {
  LayoutDashboard, IndianRupeeIcon, UserPlus, Users, Briefcase,
  BriefcaseMedical, List, CreditCard, Wallet, Building2, UserCircle,
...
export default function Sidebar() {
  ...
  const write_access = useSelector((state) => state.user?.currentUser?.data?.allow_write_access);
  ...
  const menuItems = [
    { name: "Dashboard", icon: <LayoutDashboard size={18} />, path: "/dashboard" },
    {
      ...
      subMenu: [
        ...(role === "admin" && write_access ? [{ name: "Add Super Admin", icon: <UserPlus size={18} />, path: "/add-new-superadmin" }] : []),
    ...
    },
    {
      ...
      matchPrefixes: ["/view-consultancy", "/edit-consultancy"],
      subMenu: [
        ...(role === "admin" && write_access ? [{ name: "Add Consultancy", icon: <BriefcaseMedical size={18} />, path: "/add-new-consultancy" }] : []),
        { name: "All Consultancies", icon: <List size={18} />, path: "/all-consultancies" },
      ],
    },
    {
      name: "Payments",
      icon: <Wallet size={18} />,
      matchPrefixes: ["/edit-a-payment", "/view-a-payment"],
      subMenu: [
        ...(role === "admin" && write_access ? [{ name: "Add New Payment", icon: <CreditCard size={18} />, path: "/add-new-payment" }] : []),
        { name: "All Payments", icon: <IndianRupeeIcon size={18} />, path: "/all-payments" },
      ],
    },
    {
      name: "Master Admin",
      ...
      subMenu: [
        ...(role === "admin" && write_access ? [{ name: "Add Master Admin", icon: <UserPlus size={18} />, path: "/add-master-admin" }] : []),
        { name: "All Master Admin", icon: <Users size={18} />, path: "/all-master-admin" },
      ],
    },
    { name: "Our Company", icon: <Building2 size={18} />, path: "/our-company" },
    { name: "My Profile", icon: <UserCircle size={18} />, path: "/my-profile" },
  ];
  ...
  const handleLogout = async () => {
    try {
      const res = await fetch("/api/super-admins/logout", {
        method: "POST",
        credentials: "include",
      });
  ...
  return (
    <>
              ...
              return (
                <li key={idx} className="relative group">
                  {hasSubMenu ? (
                    <>
                      ...
                      <ul className={`ml-10 mt-2 space-y-1 overflow-hidden transition-[max-height] duration-300 ${activeMenu === item.name && isOpen ? "max-h-64" : "max-h-0"}`}>
                        {item.subMenu.map((sub, subIdx) => (
                          <li key={subIdx}>
                            <Link href={sub.path} className={`flex items-center p-2 rounded-md ${pathname === sub.path ? "bg-[#475569] text-yellow-300 font-semibold" : "hover:bg-[#334155]"}`}>
                    ...
                    <Link href={item.path} className={`flex items-center p-2 rounded-md ${isMenuActive ? "bg-[#475569] text-yellow-300 font-semibold" : "hover:bg-[#334155]"}`}>
            ...
            {bottomMenu.map((item, idx) => (
              <li key={idx}>
                {item.action === "logout" ? (
                  ...
                  <Link href={item.path} className="flex items-center p-2 rounded-md cursor-pointer hover:bg-[#334155]">
                    <span className="flex justify-center w-6">{item.icon}</span>
                    {isOpen && <span className="ml-3 text-sm">{item.name}</span>}
                  </Link>
                )}
      ...
      {showLogoutSuccess && (
        <div className="fixed inset-0 backdrop-blur-md flex justify-center items-center z-50">
          <div className="bg-white text-[#334155] rounded-lg p-6 w-80 shadow-xl text-center">
            <p className="mb-6 text-xl font-medium">You have logged out successfully! You will be redirected to login page now.</p>
            <div className="flex justify-center gap-4">
